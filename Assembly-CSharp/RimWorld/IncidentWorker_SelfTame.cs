...
public class IncidentWorker_SelfTame : IncidentWorker
{
	...
	{
		return map.mapPawns.AllPawnsSpawned.Where((Pawn x) => x.IsNonMutantAnimal && x.Faction == null && !x.Position.Fogged(x.Map) && !x.InMentalState && !x.Downed && x.RaceProps.wildness > 0f);
	}
