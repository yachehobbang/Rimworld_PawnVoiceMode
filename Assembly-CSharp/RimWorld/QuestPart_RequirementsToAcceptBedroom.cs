...
public class QuestPart_RequirementsToAcceptBedroom : QuestPart_RequirementsToAccept
{
	...
	private List<Pawn> CulpritsAre()
	{
		...
		if (targetPawns.Any())
		{
			...
			{
				if (allMapsCaravansAndTravelingTransportPods_Alive_Colonist.royalty?.HighestTitleWithBedroomRequirements() != null && !allMapsCaravansAndTravelingTransportPods_Alive_Colonist.Suspended && (!allMapsCaravansAndTravelingTransportPods_Alive_Colonist.royalty.HasPersonalBedroom() || allMapsCaravansAndTravelingTransportPods_Alive_Colonist.royalty.AnyUnmetBedroomRequirements()))
				{
