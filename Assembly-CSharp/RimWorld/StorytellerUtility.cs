...
public static class StorytellerUtility
{
	...
	private static readonly SimpleCurve PointsFactorForColonyMutantsCurve = new SimpleCurve
	{
		new CurvePoint(0f, 0.2f),
		new CurvePoint(10000f, 0.2f),
		new CurvePoint(400000f, 0.3f),
	...
	{
		if (target is Map { IsPocketMap: not false })
		{
		...
		foreach (Pawn item in target.PlayerPawnsForStoryteller)
		{
			...
			else if (item.IsNonMutantAnimal && item.Faction == Faction.OfPlayer && !item.Downed && item.training.CanAssignToTrain(TrainableDefOf.Release).Accepted)
			{
			...
			else if (item.IsMutant)
			{
				num3 = item.kindDef.combatPower * PointsFactorForColonyMutantsCurve.Evaluate(playerWealthForStoryteller);
			}
			if (num3 > 0f)
			{
