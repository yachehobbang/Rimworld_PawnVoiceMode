...
public class WildPlantSpawner : IExposable
{
	...
	public bool CheckSpawnWildPlantAt(IntVec3 c, float plantDensity, float wholeMapNumDesiredPlants, bool setRandomGrowth = false)
	{
		...
		if (setRandomGrowth)
		{
			plant.Growth = Mathf.Clamp01(InitialGrowthRandomRange.RandomInRange);
	...
	private float PlantChoiceWeight(ThingDef plantDef, IntVec3 c, Dictionary<ThingDef, float> distanceSqToNearbyClusters, float wholeMapNumDesiredPlants, float plantDensity)
	{
		...
		float num2 = 0.5f;
		if ((float)map.listerThings.ThingsInGroup(ThingRequestGroup.NonStumpPlant).Count > wholeMapNumDesiredPlants / 2f && !plantDef.plant.cavePlant)
		{
			num2 = (float)map.listerThings.ThingsOfDef(plantDef).Count / (float)map.listerThings.ThingsInGroup(ThingRequestGroup.NonStumpPlant).Count / commonalityPctOfPlant;
	...
	{
		return Mathf.Min(GetBaseDesiredPlantsCountAt(c) * plantDensity * map.fertilityGrid.FertilityAt(forCell), 1f);
	}

	public float GetDesiredPlantsCountIn(Region reg, IntVec3 forCell, float plantDensity)
	{
		return Mathf.Min(reg.GetBaseDesiredPlantsCount() * plantDensity * map.fertilityGrid.FertilityAt(forCell), reg.CellCount);
	}
}
