...
public class Dialog_Trade : Window
{
	...
	public override void DoWindowContents(Rect inRect)
	{
		...
		if (Widgets.ButtonText(rect5, TradeSession.giftMode ? ("OfferGifts".Translate() + " (" + FactionGiftUtility.GetGoodwillChange(TradeSession.deal.AllTradeables, TradeSession.trader.Faction).ToStringWithSign() + ")") : "AcceptButton".Translate(), drawBackground: true, doMouseoverSound: true, active: true, null))
		{
		...
		if (Widgets.ButtonText(new Rect(rect5.x - 10f - OtherBottomButtonSize.x, rect5.y, OtherBottomButtonSize.x, OtherBottomButtonSize.y), "ResetButton".Translate(), drawBackground: true, doMouseoverSound: true, active: true, null))
		{
		...
		if (Widgets.ButtonText(new Rect(rect5.xMax + 10f, rect5.y, OtherBottomButtonSize.x, OtherBottomButtonSize.y), "CancelButton".Translate(), drawBackground: true, doMouseoverSound: true, active: true, null))
		{
			Close();
			Event.current.Use();
		}
		...
		{
			Rect rect7 = new Rect(rect6.x - y - 4f, rect5.y, y, y);
			if (TradeSession.giftMode)
			{
				if (Widgets.ButtonImageWithBG(rect7, TradeModeIcon, new Vector2(32f, 32f)))
				{
				...
				TooltipHandler.TipRegionByKey(rect7, "TradeModeTip");
			}
			else
			{
				if (Widgets.ButtonImageWithBG(rect7, GiftModeIcon, new Vector2(32f, 32f)))
				{
				...
				TooltipHandler.TipRegionByKey(rect7, "GiftModeTip", faction.Name);
			}
		}
		Widgets.EndGroup();
	}
