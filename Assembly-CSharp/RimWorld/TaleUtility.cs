...
public static class TaleUtility
{
	...
	public static void Notify_PawnDied(Pawn victim, DamageInfo? dinfo)
	{
		...
		if ((pawn != null && pawn.CurJob != null && pawn.jobs.curDriver is JobDriver_Execute) || victim.IsMutant)
		{
			return;
		}
		...
		if (pawn != null)
		{
			...
			else if (victim.Faction == Faction.OfPlayer && victim.IsNonMutantAnimal && !flag)
			{
