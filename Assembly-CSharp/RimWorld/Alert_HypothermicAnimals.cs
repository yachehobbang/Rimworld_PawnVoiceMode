...
public class Alert_HypothermicAnimals : Alert
{
	...
	private List<Pawn> HypothermicAnimals
	{
		get
		{
			...
			{
				if (allMaps_Spawned[i].IsNonMutantAnimal && allMaps_Spawned[i].Faction == null && allMaps_Spawned[i].health.hediffSet.GetFirstHediffOfDef(HediffDefOf.Hypothermia) != null)
				{
