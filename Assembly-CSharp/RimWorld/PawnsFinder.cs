...
public static class PawnsFinder
{
	...
	private static List<Pawn> allMaps_ColonyMutantsSpawned_Result = new List<Pawn>();

	private static List<Pawn> allMapsCaravansAndTravelingTransportPods_Alive_ColonyMutants_NoSuspended_Result = new List<Pawn>();
	...
	public static List<Pawn> AllMaps_ColonyMutantsSpawnedPlayerControlled
	{
		get
		{
			allMaps_ColonyMutantsSpawned_Result.Clear();
			if (Current.ProgramState != 0)
			{
				List<Map> maps = Find.Maps;
				if (maps.Count == 1)
				{
					return maps[0].mapPawns.SpawnedColonyMutantsPlayerControlled;
				}
				for (int i = 0; i < maps.Count; i++)
				{
					allMaps_ColonyMutantsSpawned_Result.AddRange(maps[i].mapPawns.SpawnedColonyMutantsPlayerControlled);
				}
			}
			return allMaps_ColonyMutantsSpawned_Result;
		}
	}

	public static List<Pawn> AllMapsCaravansAndTravelingTransportPods_Alive_ColonyMutants_NoSuspended
	{
		get
		{
			allMapsCaravansAndTravelingTransportPods_Alive_ColonyMutants_NoSuspended_Result.Clear();
			List<Pawn> allMapsCaravansAndTravelingTransportPods_Alive_OfPlayerFaction = AllMapsCaravansAndTravelingTransportPods_Alive_OfPlayerFaction;
			for (int i = 0; i < allMapsCaravansAndTravelingTransportPods_Alive_OfPlayerFaction.Count; i++)
			{
				if (allMapsCaravansAndTravelingTransportPods_Alive_OfPlayerFaction[i].IsColonyMutant && !allMapsCaravansAndTravelingTransportPods_Alive_OfPlayerFaction[i].Suspended)
				{
					allMapsCaravansAndTravelingTransportPods_Alive_ColonyMutants_NoSuspended_Result.Add(allMapsCaravansAndTravelingTransportPods_Alive_OfPlayerFaction[i]);
				}
			}
			return allMapsCaravansAndTravelingTransportPods_Alive_ColonyMutants_NoSuspended_Result;
		}
	}
