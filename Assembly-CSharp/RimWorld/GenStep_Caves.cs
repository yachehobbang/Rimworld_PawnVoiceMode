...
{
	private float branchChance = 0.1f;

	private float widthOffsetPerCell = 0.034f;

	private float minTunnelWidth = 1.4f;
	...
	protected const float DirectionNoiseFrequency = 0.00205f;
	...
	protected void Dig(IntVec3 start, float dir, float width, List<IntVec3> group, Map map, bool closed, HashSet<IntVec3> visited = null)
	{
		...
		while (true)
		{
			...
			if (num2 >= 15 && width > minTunnelWidth + BranchedTunnelWidthOffset.max)
			{
