...
public abstract class Page : Window
{
	...
	protected void DoBottomButtons(Rect rect, string nextLabel = null, string midLabel = null, Action midAct = null, bool showNext = true, bool doNextOnKeypress = true)
	{
		...
		if ((Widgets.ButtonText(new Rect(rect.x, y, BottomButSize.x, BottomButSize.y), label, drawBackground: true, doMouseoverSound: true, active: true, null) || KeyBindingDefOf.Cancel.KeyDownEvent) && CanDoBack())
		{
			DoBack();
		}
		if (showNext)
		{
			...
			if ((Widgets.ButtonText(rect2, nextLabel, drawBackground: true, doMouseoverSound: true, active: true, null) || (doNextOnKeypress && KeyBindingDefOf.Accept.KeyDownEvent)) && CanDoNext())
			{
				DoNext();
			}
			UIHighlighter.HighlightOpportunity(rect2, "NextPage");
		}
		if (midAct != null && Widgets.ButtonText(new Rect(rect.x + rect.width / 2f - BottomButSize.x / 2f, y, BottomButSize.x, BottomButSize.y), midLabel, drawBackground: true, doMouseoverSound: true, active: true, null))
		{
			midAct();
		}
	}
