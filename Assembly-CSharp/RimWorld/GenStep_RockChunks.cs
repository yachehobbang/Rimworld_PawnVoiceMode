...
public class GenStep_RockChunks : GenStep
{
	...
	public override void Generate(Map map, GenStepParams parms)
	{
		...
		foreach (IntVec3 allCell in map.AllCells)
		{
			...
			if ((elevation[allCell] < 0.55f || map.generatorDef.isUnderground) && Rand.Value < num)
			{
	...
	private void GrowLowRockFormationFrom(IntVec3 root, Map map)
	{
		...
		while (true)
		{
			...
			if (!intVec.InBounds(map) || intVec.GetEdifice(map) != null || intVec.GetFirstItem(map) != null || (!map.generatorDef.isUnderground && elevation[intVec] > 0.55f) || !map.terrainGrid.TerrainAt(intVec).affordances.Contains(TerrainAffordanceDefOf.Heavy))
			{
				break;
			}
