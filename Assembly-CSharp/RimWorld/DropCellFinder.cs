...
public static class DropCellFinder
{
	...
	{
		return CellFinderLoose.RandomCellWith((IntVec3 c) => (!standableOnly || c.Standable(map)) && !c.Roofed(map) && !c.Fogged(map), map);
	}
	...
	public static IntVec3 FindRaidDropCenterDistant(Map map, bool allowRoofed = false)
	{
		...
		while (true)
		{
			...
			if (!CanPhysicallyDropInto(intVec, map, canRoofPunch: true, allowedIndoors: false) || intVec.Fogged(map) || (!allowRoofed && intVec.Roofed(map)))
			{
				continue;
			}
			num2 -= 0.2f;
