...
public class RegionCostCalculator
{
	...
	{
		int num = pathingContext.pathGrid.pathGrid[index];
		if (avoidGrid != null)
		{
	...
	private List<int> PathableNeighborIndices(int index)
	{
		...
		bool flag4 = !num || PathFinder.BlocksDiagonalMovement(index - 1, pathingContext, canBashFences);
		bool flag5 = !flag || PathFinder.BlocksDiagonalMovement(index + 1, pathingContext, canBashFences);
		if (flag2 && !PathFinder.BlocksDiagonalMovement(index - x, pathingContext, canBashFences))
		{
		...
		if (flag3 && !PathFinder.BlocksDiagonalMovement(index + x, pathingContext, canBashFences))
		{
			if (!flag5 && pathGrid.WalkableFast(index + x + 1))
			{
				tmpPathableNeighborIndices.Add(index + x + 1);
