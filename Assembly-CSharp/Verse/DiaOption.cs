...
public class DiaOption
{
	...
	public float OptOnGUI(Rect rect, bool active = true)
	{
		...
		{
			Widgets.HyperlinkWithIcon(rect, hyperlink, text, 2f, 6f, null);
		}
		else if (Widgets.ButtonText(rect, text, drawBackground: false, !disabled, textColor, active && !disabled, null))
		{
			Activate();
		}
		return rect.height;
