...
public class Explosion : Thing
{
	...
	private void AffectCell(IntVec3 c)
	{
		...
		if (!flag)
		{
			...
			{
				ThingDef thingDef = (c.GetTerrain(base.Map).IsWater ? (postExplosionSpawnThingDefWater ?? postExplosionSpawnThingDef) : postExplosionSpawnThingDef);
				TrySpawnExplosionThing(thingDef, c, postExplosionSpawnThingCount);
			}
			if (postExplosionGasType.HasValue)
			{
				GasUtility.AddGas(c, base.Map, postExplosionGasType.Value, 255);
			}
		}
