...
public static class DebugOutputsTechnical
{
	...
	public static void BestThingRequestGroup()
	{
			...
			select x, new TableDataGetter<ThingDef>("defName", (ThingDef d) => d.defName), new TableDataGetter<ThingDef>("best local", delegate(ThingDef d)
		{
			...
			return string.Concat(best, " (defs: ", DefDatabase<ThingDef>.AllDefs.Count((ThingDef x) => ListerThings.EverListable(x, ListerThingsUse.Region) && best.Includes(x)), ")");
		}), new TableDataGetter<ThingDef>("best global", delegate(ThingDef d)
		{
			...
			return string.Concat(best2, " (defs: ", DefDatabase<ThingDef>.AllDefs.Count((ThingDef x) => ListerThings.EverListable(x, ListerThingsUse.Global) && best2.Includes(x)), ")");
		}));
	}
