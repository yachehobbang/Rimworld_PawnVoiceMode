...
public class ThingFilter : IExposable
{
	...
	public string Summary
	{
		get
		{
			...
			if (thingDefs != null && thingDefs.Count == 1 && categories.NullOrEmpty() && tradeTagsToAllow.NullOrEmpty() && tradeTagsToDisallow.NullOrEmpty() && thingSetMakerTagsToAllow.NullOrEmpty() && thingSetMakerTagsToDisallow.NullOrEmpty() && disallowedCategories.NullOrEmpty() && specialFiltersToAllow.NullOrEmpty() && specialFiltersToDisallow.NullOrEmpty() && stuffCategoriesToAllow.NullOrEmpty() && allowAllWhoCanMake.NullOrEmpty() && disallowWorsePreferability == FoodPreferability.Undefined && !disallowInedibleByHuman && !disallowDoesntProduceMeat && !disallowNotEverStorable && allowWithComp == null && disallowWithComp == null && disallowCheaperThan == float.MinValue && disallowedThingDefs.NullOrEmpty())
			{
	...
	public void ResolveReferences()
	{
		...
		if (disallowDoesntProduceMeat)
		{
			...
			for (int num13 = 0; num13 < allDefsListForReading7.Count; num13++)
			{
				...
				if (thingDef7.race != null && !thingDef7.race.hasMeat)
				{
		...
		if (allowWithComp != null)
		{
			...
			for (int num14 = 0; num14 < allDefsListForReading8.Count; num14++)
			{
				...
				if (thingDef8.HasComp(allowWithComp))
				{
					SetAllow(thingDef8, allow: true);
				}
			}
		}
		if (disallowWithComp != null)
		{
			...
			for (int num15 = 0; num15 < allDefsListForReading9.Count; num15++)
			{
				...
				if (thingDef9.HasComp(disallowWithComp))
				{
					SetAllow(thingDef9, allow: false);
				}
			}
		}
		if (disallowCheaperThan != float.MinValue)
		{
			...
			{
				SetAllow(list[num16], allow: false);
			}
		}
		...
		for (int num17 = 0; num17 < disallowedThingDefs.Count; num17++)
		{
			if (disallowedThingDefs[num17] != null)
			{
				SetAllow(disallowedThingDefs[num17], allow: false);
			}
			else
			{
				Log.Error("ThingFilter could not find excepted thing def named " + disallowedThingDefs[num17]);
			}
		}
	}
