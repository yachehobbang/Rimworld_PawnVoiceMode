	...
	private void CalculateContiguousGroups()
	{
		...
		for (int k = 0; k < roots.Count; k++)
		{
			...
			for (int l = 0; l < currentGroup.Count; l++)
			{
				...
				if (IsMember(num3, out var ifRootThenRootGroupSizeMustMatch3) && (!ifRootThenRootGroupSizeMustMatch3 || !rootsSet.Contains(num3) || (FeatureWorker.groupSize[num3] >= minRootGroupSize && FeatureWorker.groupSize[num3] <= maxRootGroupSize)))
				{
