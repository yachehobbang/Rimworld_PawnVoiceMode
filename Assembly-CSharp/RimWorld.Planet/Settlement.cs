...
public class Settlement : MapParent, ITrader, ITraderRestockingInfoProvider
{
	...
	public override IEnumerable<FloatMenuOption> GetShuttleFloatMenuOptions(IEnumerable<IThingHolder> pods, Action<int, TransportPodsArrivalAction> launchAction)
	{
		...
		if ((bool)TransportPodsArrivalAction_Trade.CanTradeWith(pods, this))
		{
			yield return new FloatMenuOption("TradeWith".Translate(Label), delegate
			{
				launchAction(base.Tile, new TransportPodsArrivalAction_Trade(this));
			});
		}
		...
		if (base.HasMap || (firstPod = pods.FirstOrDefault() as CompTransporter) == null || firstPod.Shuttle.shipParent == null)
		{
			yield break;
		}
		foreach (FloatMenuOption floatMenuOption in TransportPodsArrivalActionUtility.GetFloatMenuOptions(() => TransportPodsArrivalAction_AttackSettlement.CanAttack(pods, this), () => new TransportPodsArrivalAction_TransportShip(this, firstPod.Shuttle.shipParent), "AttackShuttle".Translate(Label), launchAction, base.Tile))
		{
			yield return floatMenuOption;
		}
	}
