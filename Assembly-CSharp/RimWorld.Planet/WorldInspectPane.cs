...
public class WorldInspectPane : Window, IInspectPane
{
	...
	private string TileInspectString
	{
		get
		{
			...
			{
				stringBuilder.AppendLine();
				stringBuilder.Append(tile.Rivers[0].river.LabelCap);
			}
			if (!Find.World.Impassable(SelectedTile))
			{
				string text = (WorldPathGrid.CalculatedMovementDifficultyAt(SelectedTile, perceivedStatic: false, null) * Find.WorldGrid.GetRoadMovementDifficultyMultiplier(SelectedTile, -1)).ToString("0.#");
				stringBuilder.AppendLine();
				stringBuilder.Append("MovementDifficulty".Translate() + ": " + text);
			}
			stringBuilder.AppendLine();
			...
			if (ModsConfig.BiotechActive)
			{
				...
				stringBuilder.Append("TilePollution".Translate() + ": " + pollutionDescription);
			}
	...
	public void DrawInspectGizmos()
	{
		...
		if (FormCaravanGizmoUtility.TryGetGizmo(out var gizmo))
		{
